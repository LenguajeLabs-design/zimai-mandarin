...
export function QuickView({ word, family, root, saved, learned, onClose, onToggleSaved, onToggleLearned, audio }: QuickViewProps) {
  return <div className="quick-view-layer" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose() }}>
    <section className="quick-view" role="dialog" aria-modal="true" aria-labelledby="quick-view-title">
      ...
      <div className="quick-view__word"><span className="quick-view__hanzi" lang="zh-CN">{highlightedCharacters(word.hanzi, root.hanzi).map(({ character, isRoot }, index) => <span className={isRoot ? 'root-char' : undefined} key={`${character}-${index}`}>{character}</span>)}</span><AudioButton audioKey={word.audioKey} text={word.hanzi} playingKey={audio.playingKey} onPlay={audio.onPlay} onStop={audio.onStop} label={`Play audio for ${word.hanzi}`} /></div>
      <p className="quick-view__pinyin">{word.pinyin}</p><p className="quick-view__gloss">{word.gloss}</p>
      {word.examples?.[0] && <div className="quick-view__example"><p className="eyebrow">In context</p><p className="quick-view__example-hanzi" lang="zh-CN">{word.examples[0].hanzi}</p><p className="quick-view__example-pinyin">{word.examples[0].pinyin}</p><p className="quick-view__example-gloss">{word.examples[0].gloss}</p></div>}
